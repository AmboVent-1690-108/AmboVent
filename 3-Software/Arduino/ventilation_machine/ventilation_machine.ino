...
int A_pot,prev_A_pot, A_current, Compression_perc=80, prev_Compression_perc, A_rate, A_comp, A_pres;
...
float wanted_pos, wanted_vel_PWM, range, range_factor, profile_planned_vel, planned_vel, integral, error,prev_error, f_reduction_up ;
...
void loop() 
{
  ...
  switch (state)
  {
    ...
    case BREATH_STATE:     // run profile
      run_profile_func ();
      if (SW2_pressed) state=STBY_STATE;  // stop breathing motion   
      break;
...
{ 
  if (millis()-lastIndex >= wanted_cycle_time)                        // do when cycle time was reached
  {
    ...
    if (index<0) 
    { 
      if(safety_pressure_detected==1) safety_pressure_counter+=1;        // count the number of cases reaching safety pressure
    ...
    { 
      ...
      {
        if (avg_pres - pressure_abs > delta_pres_patient_inhale) start_new_cycle();    // start new breath cycle if patient tries to inhale durint the PEEP plateu
...
void blink_user_led()
{
  ...
  { 
    if (index>0.1*profile_length) LED_USR(0); 
    else LED_USR(1); 
  }  
}
...
{
  if (millis()-lastUSRblink>100) {lcd.setCursor(13, 0); lcd.print(A_pot); lcd.print(" "); lastUSRblink=millis();}
}
...
{ 
  ...
  while (progress==0) internal_arm_calib_step();  // step 1 - calibrate top position
  progress = 0;
  min_arm_pos=A_pot;

  display_text_calib ("Set Lower");
  while (progress==0) internal_arm_calib_step();  // step 2 - calibrate bottom position
  progress = 0;
  max_arm_pos=A_pot; 

  display_text_calib ("Move to Safe");
  while (progress==0) internal_arm_calib_step();  // step 3 - manual control for positioning back in safe location 
...
{ 
  ...
  while (TST_pressed==0) read_IO ();    // step 1 - calibrate top position
  ...
  while (TST_pressed==0)  read_IO ();    // step 2 - calibrate bottom position
...
{
  if (abs(A_pot-prev_A_pot)>0 || abs(wanted_vel_PWM)<15) index_last_motion=index;
...
{ 
  ...
  if (TST==0 && prev_TST==1) TST_pressed=1; 
  else TST_pressed=0;  
  ...
  {   
    ...
    if (TST==0) 
    {
      if (FD==0 && prev_FD==1) {BPM-=2; if(BPM<6) BPM=6; cycle_number=0;} 
      if (FU==0 && prev_FU==1) {BPM+=2; if(BPM>24) BPM=24; cycle_number=0;}
      breath_cycle_time = 60000/BPM+100;
      if (AD==0 && prev_AD==1) {Compression_perc-=deltaUD; if (Compression_perc<perc_of_lower_vol_display) Compression_perc=perc_of_lower_vol_display; }
      if (AU==0 && prev_AU==1) {Compression_perc+=deltaUD; if (Compression_perc>100) Compression_perc=100;}
    }
    if (TST==1) 
    {
      if (FD==0 && prev_FD==1) {insp_pressure-=5; if(insp_pressure<30) insp_pressure=30; } 
      if (FU==0 && prev_FU==1) {insp_pressure+=5; if(insp_pressure>70) insp_pressure=70;}
      if (AD==0 && prev_AD==1) {insp_pressure-=5; if(insp_pressure<30) insp_pressure=30; } 
      if (AU==0 && prev_AU==1) {insp_pressure+=5; if(insp_pressure>70) insp_pressure=70;}
    }
  }
  range_factor = perc_of_lower_volume+(Compression_perc-perc_of_lower_vol_display)*(100-perc_of_lower_volume)/(100-perc_of_lower_vol_display);
  ...
  if (wanted_cycle_time > breath_cycle_time/profile_length) wanted_cycle_time=breath_cycle_time/profile_length;
  if (wanted_cycle_time < cycleTime) wanted_cycle_time=cycleTime;
}
